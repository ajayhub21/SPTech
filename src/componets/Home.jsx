      ...
      <div className="py-16 bg-gradient-to-b from-blue-50 to-white min-h-screen flex items-center">
        <div className="container mx-auto px-6 flex flex-col lg:flex-row items-center lg:space-x-12">
          ...
          <div className="lg:w-1/2 mt-8 lg:mt-0 flex justify-center relative">
            <div className="relative w-full max-w-md transform animate-slideIn">
              <img
                src="/SPTech/HomeSide1"
                alt="Document Scanning Solutions"
                className="w-full relative h-auto max-w-[90%] md:max-w-[75%] rounded-xl shadow-lg object-cover transition duration-300 hover:scale-105"
              />
            </div>
          </div>
        </div>
      ...
      {sections.map(({ component, ref }, index) => (
        <div
          ...
          className={`transition duration-1000 transform ${
            useAnimateOnView(ref) ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
          }`}
        >
          {component}
        </div>
